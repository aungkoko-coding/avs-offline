...
import { checkGenresInList, checkItemInList, fetchFromApi, searchForBoth, searchForMovies, searchForTvShows } from '../api/Util';

const genres = ["Action", "Comedy", "Horror", "Thriller", "Adventure", "Sci-Fi", "Fantasy"];
...
export default function ContextProvider(props) {
    ...
    const [strictMode, setStrictMode] = useState(JSON.parse(localStorage.getItem("strictMode")) || false);
    const [multipleSearchMode, setMultipleSearchMode] = useState(JSON.parse(localStorage.getItem("multipleSearchMode")) || false);
    const [searchBy, setSearchBy] = useState(localStorage.getItem("searchBy") || "title");
    const [searchFor, setSearchFor] = useState(localStorage.getItem("searchFor") || "both");
    ...
    const removeFromCart = async (item) => {
        ...
        if (isItemExist) {
            setCartItems(items => {
                let updatedItems = items.filter(i => i.slug.current !== item.slug.current)
                localStorage.setItem('cart', JSON.stringify(updatedItems));
                return updatedItems;
            });
            return true;
            ...
            if (searchFor === searchForMovies) {
                ...
                result.forEach(data => {
                    if (data.type === searchForMovies) {
                        const movies = data.movies?.filter(movie => movie.genres?.includes(genre));
                    ...
                    if (data.type === searchForTvShows) {
                        const filteredTvShows = data.tvShows?.filter(tvShow => tvShow.genres?.includes(genre));
    ...
    const searchByGenres = async (genres, searchFor) => {
        ...
        if (result && result.length >= 1) {
            ...
            } else if (searchFor === searchForBoth) { // search for both
                let res = [];
                result.forEach(data => {
                    if (data.type === searchForMovies) {
        ...
        if (result && result.length >= 1) {
            if (searchFor === searchForMovies) {
                return result.filter(movie => {
                    ...
                    return strictMode ? movieTitle === searchTitle : searchTitle.includes(movieTitle) || movieTitle.includes(searchTitle);
                });
            } else if (searchFor === searchForTvShows) {
                return result.filter(tvShow => {
                    const tvShowTitle = tvShow.title.toString().toLowerCase();
                    const searchTitle = title.toString().toLowerCase();
                    return strictMode ? tvShowTitle === searchTitle : searchTitle.includes(tvShowTitle) || tvShowTitle.includes(searchTitle);
                });
            } else if (searchFor === searchForBoth) {
                let finalResult = [];
                result.forEach(res => {
    ...
    const searchByTitles = async (titles, searchFor) => {
        ...
        if (result && result.length >= 1) {
            if (searchFor === searchForMovies) {
                ...
                let finalResult = [];
                const filteredMovies = result.find(res => res.type === searchForMovies).movies.filter(movie => {
                    if (strictMode) {
                        for (const title of titles) {
                            if (title.toLowerCase() === movie.title.toLowerCase()) {
                                return true;
                ...
                const filteredTvShows = result.find(res => res.type === searchForTvShows).tvShows.filter(tvShow => {
                    if (strictMode) {
                        for (const title of titles) {
                            if (title.toLowerCase() === tvShow.title.toLowerCase()) {
                                return true;
    ...
    async function getLastAddedMoviesAndTvShows(count) {
        ...
        const now = new Date();
        const last30Days = new Date(`${now.getFullYear()}/${now.getMonth()}/${now.getDate()}`);
