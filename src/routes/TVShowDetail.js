...
export default function TVShowDetail(props) {
  ...
  const seasonQuery = querySeason(slug);
  const {
    data: season,
    isSuccess,
    isError,
  } = useQuery({
    queryKey: [seasonQuery],
    queryFn: () => client.fetch(seasonQuery),
    enabled: !!slug,
  });
  ...
  }, [isSuccess, networkError, closeNetworkError, season]);
