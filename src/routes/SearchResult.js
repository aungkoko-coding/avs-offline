...
import { Box, CircularProgress, Divider, LinearProgress, Pagination, PaginationItem, Portal, ToggleButton, ToggleButtonGroup, Typography } from '@mui/material';
import { Link, useSearchParams } from 'react-router-dom';
import { AppContext } from '../context/AppContextProvider';
import { searchForBoth, searchForMovies, searchForTvShows, sortStringInAscendingOrder, sortStringInDescendingOrder } from '../api/Util';
...
export default function SearchResult() {
    ...
    const { networkError, searchingIndicator, searchFor: defaultSearchFor, closeSearchingIndicator, searchByGenre, searchByGenres,
        searchByTitle, searchByTitles, closeNetworkError, openNetworkError, openSearchingIndicator } = useContext(AppContext);
    ...
    //         console.log(prevGenre, prevTitle);
           
    //         if(JSON.stringify(byGenre) !== prevGenre || JSON.stringify(byTitle) !== prevTitle) {
    //             console.log('open')
    ...
    useEffect(() => {
        if (paginationResult) {
            ...
            (async () => {
                ...
                openSorting();
                if (sortBy === ASCE) {
                    res = paginationResult.sort((item1, item2) => sortStringInAscendingOrder(item1.title, item2.title));
                } else if (sortBy === DESC) {
                    res = paginationResult.sort((item1, item2) => sortStringInDescendingOrder(item1.title, item2.title));
                }
                return res;
            })().then(res => {
    ...
    useEffect(() => {
        ...
        const handleResult = (res) => {
            let result = res;
            if (sortBy === ASCE) {
                result = res.sort((item1, item2) => sortStringInAscendingOrder(item1.title, item2.title));
            } else if (sortBy === DESC) {
                result = res.sort((item1, item2) => sortStringInDescendingOrder(item1.title, item2.title));
            }
        ...
        if (searchingIndicator || (JSON.stringify(byTitle) !== prevTitle || JSON.stringify(byGenre) !== prevGenre) && !result) {
        ...
        if (itemsPerPage && result) {
            ...
            if(sortBy) {
                (async () => {
                    openSorting();
                    if (sortBy === ASCE) {
                        paginationResult = paginationResult.sort((item1, item2) => sortStringInAscendingOrder(item1.title, item2.title));
                    } else if (sortBy === DESC) {
                        paginationResult = paginationResult.sort((item1, item2) => sortStringInDescendingOrder(item1.title, item2.title));
                    }
        ...
        <Box position='relative' width='100%' p={1} px={{ xs: 0.3, sm: 3, md: 5, lg: 7 }}>
            <Box mt={0.5} display='flex' justifyContent='space-between'>
                ...
                <Box display='flex' alignItems='center' flexShrink={0}>
                    <ToggleButtonGroup disabled={searchingIndicator} size='small' exclusive value={sortBy} onChange={handleSort}>
            ...
            <Divider sx={{ my: 1 }} />
            <Box sx={{ mt: 2, width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', height: 'auto' }}>
                {
            ...
            </Box>
            {
                ...
                <Portal container={appRoot} children={<BusyIndicator shape={sorting ? 'circular' : 'linear'} message={sorting ? '' : 'Searching...'} />} />
            }
