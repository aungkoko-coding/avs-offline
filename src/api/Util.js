...
export async function fetchFromApi(queryFor = searchForBoth) {
    ...
    switch (queryFor) {
        ...
        case searchForBoth:
            ...
            result = result.concat({ type: 'movies', movies }).concat({ type: 'tvShows', tvShows });
            break;
        default: throw new Error("Did not get what to query for!");
    }

    return result;
}

export function checkGenresInList(strictMode = false, genresList, searchGenres) {
    ...
    if (strictMode) {
        ...
        for (let i = 0; i < searchGenresLength; i++) {
            ...
            const genre = searchGenre.slice(0, 1).toUpperCase() + searchGenre.slice(1).toLowerCase();
            if (genresList?.includes(genre)) {
                return true;
            } else {
    ...
    try{
        if(promotion) {
            ...
            if(from <= current && to >= current) {
                calculatedPrice = price - (parseFloat(price) / 100 * parseInt(promotion?.discount))
            }
        }
