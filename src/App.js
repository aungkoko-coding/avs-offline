...
import { AppBar, Link as MuiLink, Badge, Box, Button, CssBaseline, IconButton, MenuItem, responsiveFontSizes, Toolbar, Typography, alpha, ButtonGroup, Popper, Paper, ClickAwayListener, MenuList, Grow, Grid, Tooltip } from '@mui/material';
...
import { queryAllCollections, queryAllMovies, queryAllSeasons, queryPromotion } from './lib/queries';
...
function App(props) {
  ...
  const [searchSettingsDialogOpen, setSearchSettingsDialogOpen] = useState(false);
  const [visited, setVisited] = useState(localStorage.getItem("visited") || false);
  const forGenresMenuRef = useRef(null);
  const forSearchInputBoxRef = useRef(null);
  const navigate = useNavigate();
  const location = useLocation();

  const { cartItems, promotion, setPromotion, genreValues, genres, tags, setTags, setGenreValues, networkError, openNetworkError, closeNetworkError, openSearchingIndicator, searchInputBoxOpen, setSearchInputBoxOpen } = useContext(AppContext);
  ...
  useEffect(() => {
    if (!genreValues || !tags) {
      ...
      client.fetch(queryAllMovies())
        ...
        .then(collections => {
          let cols = collections?.map(collection => ({ type: 'collection', value: collection.name, slug: collection.slug.current }));
          let genres = Object.getOwnPropertyNames(genreValues).map(genre => ({ type: 'genre', value: genre }))
  ...
  function handleGenresMenuClose(event) {
    if (forGenresMenuRef.current && forGenresMenuRef.current.contains(event.target)) {
      return;
    }
    setOpenMenu(false);
  }
  ...
  function closeSearchInputBox(event) {
    if (event && event.target && forSearchInputBoxRef.current && forSearchInputBoxRef.current.contains(event.target)) {
      return;
    }
    ...
    <ThemeProvider theme={responsiveTheme}>
      ...
      <AppBar position="fixed">
        <Toolbar>
          ...
          <Grid container flexGrow={1} width="unset" sx={{ display: { xs: 'none', md: 'flex' } }}>
                      ...
                      <MenuList autoFocusItem>
                        {
                          ((genreValues && Object.getOwnPropertyNames(genreValues)) || genres).map((genre, index) => (
                            <MenuItem key={index} onClick={(e) => { handleGenresMenuClose(e); handleSearchByGenre(genre); }}>
                              <SearchIcon fontSize="small" />
                              <Typography variant="subtitle1" sx={{ ml: 1 }}>{genre}</Typography>
                            </MenuItem>
                          ))
                        }
                        {
          ...
          <Box>
            <Tooltip title='Search'>
              <IconButton onClick={toggleSearchInputBox} ref={forSearchInputBoxRef}>
                {
            ...
            <Tooltip title='Cart'>
              <IconButton onClick={() => navigate('/cart')}>
                <Badge badgeContent={cartItems ? cartItems.length : 0} showZero sx={{ '& .MuiBadge-badge': { bgcolor: "#ff4081" } }}>
                  <ShoppingCartIcon />
                </Badge>
              </IconButton>
            </Tooltip>
        ...
        <SearchSettingsDialog open={searchSettingsDialogOpen} onClose={() => setSearchSettingsDialogOpen(false)} />
      </AppBar>

      <Box sx={{ pt: { xs: '56px', sm: '64px' }, height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        {
          // we don't want to display this network error message when we are in search page
          // because it handle own error message
          networkError && (location.pathname !== '/search' && location.pathname !== '/cart') &&
